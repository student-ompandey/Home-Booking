...
import { Heart, Menu, X, Plus, User, LogOut, Home, Shield } from 'lucide-react';
                ...
                {profileOpen && (
                  <>
                    ...
                    <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-lg border border-gray-border py-2 animate-fade-in">
                      ...
                      {isOwner && (
                        <Link to="/my-listings" onClick={() => setProfileOpen(false)} className="flex items-center gap-2 px-4 py-2.5 text-sm text-dark hover:bg-gray-light transition-colors">
                          <Home className="w-4 h-4" /> My Listings
                        </Link>
                      )}
                      {isAdmin && (
                        <a href="http://localhost:3002/admin" className="flex items-center gap-2 px-4 py-2.5 text-sm text-primary font-medium hover:bg-primary/5 transition-colors">

...
class RoomService {
  ...
  static async getRooms(queryParams) {
    ...
    // Build filter object — public API only shows approved rooms
  ...
  static async createRoom(roomData, ownerId) {
    const room = await Room.create({ ...roomData, owner: ownerId, status: "pending" });
    logger.info(`Room created (pending approval): "${room.title}" by owner ${ownerId}`);
    return room;
  }
